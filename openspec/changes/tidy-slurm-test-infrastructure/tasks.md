...
- [ ] 1.1 Add a superseded pointer to the top of
...
- [ ] 1.2 Correct the `slurm_container` fixture docstring in `tests/conftest.py`
...
- [ ] 1.3 Record the registry findings in the pointer added in 1.1: anonymous
...
- [ ] 2.1 Delete `discover_container_port()` from `tests/slurm_support.py`;
  verify with `grep -rn discover_container_port` returning no matches and
  `uv run ruff check` passing.
- [ ] 2.2 Delete the `MEMORY` computation and its `sed` substitution from
...
- [ ] 3.1 Confirm no source module, dependency, entry point or public API was
  touched: `git diff --name-only` lists only files under `tests/` and
  `openspec/changes/archive/`.
- [ ] 3.2 Run `uv run pytest -m "not containerized"` and `uv run ruff check`
  and confirm both pass with the same results as before the change.
- [ ] 3.3 Run the containerized tests on a machine with a working container
